...
int CorMat_3(float* upper_tri, float * BOLD, int N, int L,long long OOO)
{
    ...
    while(flag==1)
    {
        ...
        cout<<"\n block and n_prime and M1 of current step: "<<block<<" "<<N_prime<<" "<<M1<<"\n";
        
        if(pak==0)//allocate devcormat and devupper in the beginning (there is no need for initializing them multiple times)
        {
            long long cormat_fullsize=block;
